---
title: Social contexts shape information gathering, attention, and memory during familiar language comprehension and novel word learning   
...
Learning a first language should be hard. Consider that even concrete nouns are often used in complex contexts with multiple possible referents, which in turn have many conceptually natural properties that a speaker could talk about. This ambiguity creates the potential for an (in principle) unlimited amount of referential uncertainty in the learning task.^[This problem is a simplified version of Quine's \textit{indeterminacy of reference} [@quine19600]: That there are many possible meanings for a word ("Gavigai") that include the referent ("Rabbit") in their extension, e.g., "white," "rabbit," "dinner." Quine's broader philosophical point was that different meanings ("rabbit" and "undetached rabbit parts") could actually be extensionally identical and thus impossible to tease apart.]. Moreover, finding meaning in language requires rapdily establishing reference during real-time interaction where the incoming information is dynamic, multimodal, and transient. Remarkably, word learning proceeds despite this uncertainty, with estimates of adult vocabularies ranging between 50,000 to 100,000 distinct lexical concepts [@bloom2002children]. How do learners infer and retain such a large variety of word meanings from data with this kind of ambiguity?

Social-pragmatic theories of language acquisition emphasize the importance of the social context for word learning [@bloom2002children; @clark2009first; @hollich2000breaking]. Experimental work has shown that even children as young as 16 months prefer to map novel words to objects that are the target of a speaker’s gaze and not their own [@baldwin1993infants]. In an analysis of naturalistic parent-child labeling events, @yu2012embodied found that young learners tended to retain labels that were accompanied by clear referential cues, which served to make a single object dominant in the visual field. And correlational studies have demonstrated strong links between early intention-reading skills (e.g., gaze following) and later vocabulary growth [@brooks2005development; @carpenter1998social]. Moreover, studies outside the domain of language acquisition have shown that the presence of social cues: (a) produce better spatial learning of audiovisual events [@wu2011infants], (b) boost recognition of a cued object [@cleveland2007joint], and (c) lead to preferential encoding of an object’s featural information [@yoon2008communication]. 

Together, the evidence suggests that social information shapes the basic information gathering processes that support  word learning. My dissertation work directly tests this information-seeking proposal. To do this, I decompose the concrete word learning problem into two components [@frank2016performance]:

  1. language comprehension
  2. mapping words to objects

In our previous work, we characterized how children and adults choose to allocate visual attention between a social partner and objects during American Sign Language (ASL) comprehension ([Chapter 1](#ch1)). We then compared the dynamics of eye movements during ASL processing to those of spoken language learners. We found that ASL-learners generated slower but more accurate gaze shifts away from a language source and produced a smaller proportion of nonlanguage-driven shifts ([Chapter 2](ch2)). To explain these modality differences, we proposed an information-seeking account and tested predictions of the account across a variety of language comprehension contexts. We found the same pattern of behavior for English-speaking adults processing displays of printed text and for children processing degraded speech. These results suggest that listeners flexibly adapt to the value of seeking higher value visual information with respect to the goal of rapid langauge comprehension.

In a separate line of work, we asked whether the presence of a social cue -- a speaker's gaze -- could change the representations underlying novel word learning ([Chapter 3](#ch3)). Our results suggest that word learneers stored representations with different levels of fidelity depending on the amount of ambiguity present during learning. In the absence of a referential cue to word meaning, learners tended to store more alternative word-object links. In contrast, when gaze was present learners stored less information, showing behavior consistent with tracking a single hypothesis. Thus, word learners flexibly respond to the amount of ambiguity in the input, and as referential uncertainty increases, they tend to store more information.  

Ib the current proposal, I present a study that connects our prior work on eye movements during familiar languge comprehension with the work on social cues during novel word learning. I will directly test predictions that our information seeking account makes for how listeners choose to allocate visual attention during novel word learning contexts. The goal is to understand ask whether learners flexibly adapt the dynamics of eye movements to seek higher value information to facilirate word learning. Overall, these results will synthesize ideas from research on active learning, social-pragmatic theories of language acqusition, and work on goal-based vision to increase our understanding of how in-the-moment processes that support languageacquisition across a wider variety of populations and learning contexts.
...
```{r sol_stimuli}
readPNG("figs/sol_fig1_trial_info.png") %>% grid.raster()
```
...
```{r sol_plot}
readPNG("figs/sol_fig2_timecourse_all.png") %>% grid.raster()
```

## Chapter 2: An information seeking account of eye movements in spoken and signed language comprehension {#ch2}
...
readPNG("figs/speed_acc_trio.png") %>% grid.raster()
```

```{r speed_acc_noise_plot}
readPNG("figs/speed_acc_noise.png") %>% grid.raster()
```

## Chapter 3: Social information modulates attention and memory during word learning {#ch3}
...
```{r soc_xsit_plot}
readPNG("figs/soc_xsit.png") %>% grid.raster()
```
...
```{r gaze_xsit_plot}
readPNG("figs/gaze_xsit.png") %>% grid.raster()
```
